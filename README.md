- [《TCP/IP 网络编程》学习笔记](#tcpip-网络编程学习笔记)
  ...
  - [第 5 章 基于 TCP 的服务端/客户端（2）](#第-5-章-基于-tcp-的服务端客户端2)
    ...
    - [5.2 TCP 原理](#52-tcp-原理)
      ...
      - [5.2.2 TCP 内部工作原理 1：与对方套接字的连接](#522-tcp-内部工作原理-1与对方套接字的连接)
  ...
  - [第 13 章 多种 I/O 函数](#第-13-章-多种-io-函数)
    - [13.1 send \& recv 函数](#131-send--recv-函数)
      ...
      - [13.1.2 MSG_OOB：发送紧急消息](#1312-msg_oob发送紧急消息)
  ...
  - [第 17 章 优于 select 的 epoll](#第-17-章-优于-select-的-epoll)
    - [17.1 epoll 理解及应用](#171-epoll-理解及应用)
      ...
      - [17.1.6 epoll_wait](#1716-epoll_wait)
...
[endian_conv.c](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch03/endian_conv.c)
...
[inet_addr.c](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch03/inet_addr.c)
...
[inet_aton.c](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch03/inet_aton.c)
...
int listen(int sockfd, int backlog);
//成功时返回0，失败时返回-1
//sockfd: 希望进入等待连接请求状态的套接字文件描述符，传递的描述符套接字参数称为服务端套接字
...
- 代码：[hello_server.c](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch04/hello_server.c)
...
- 代码：[hello_client.c](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch04/hello_client.c)
...
- [echo_server.c](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch04/echo_server.c)
- [echo_client.c](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch04/echo_client.c)
...
[本章代码](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch05/README.md)
...
- [echo_client2.c](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch05/echo_client2.c)

这样修改为了接收所有传输数据而循环调用 `read` 函数。测试及运行结果可参考第四章。

#### 5.1.3 如果问题不在于回声客户端：定义应用层协议

**回声客户端可以提前知道接收数据的长度，这在大多数情况下是不可能的**。那么此时无法预知接收数据长度时应该如何收发数据？这时需要的是**应用层协议**的定义。在收发过程中定好规则（协议）以表示**数据边界**，或者提前告知需要发送的数据的大小。服务端/客户端实现过程中逐步定义的规则集合就是应用层协议。
...
- [My_op_server.c](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch05/My_op_server.c)
- [My_op_client.c](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch05/My_op_client.c)
...
- [op_client.c](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch05/op_client.c)
- [op_server.c](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch05/op_server.c)
...
通过第一步三次握手过程完成了数据交换准备，下面就**开始正式收发数据**，其默认方式如图所示：
...
图中数据包内的 `FIN` 表示断开连接。也就是说，双方各发送 `1` 次 `FIN` 消息后断开连接。此过过程经历 4 个阶段，因此又称**四次挥手**（Four-way handshaking）。`SEQ` 和 `ACK` 的含义与之前讲解的内容一致，省略。图中，主机 `A` 传递了两次 `ACK` `5001`，也许这里会有困惑。其实，第二次 `FIN` 数据包中的 `ACK` `5001` 只是因为接收了 `ACK` 消息后未接收到的数据重传的。

## 第 6 章 基于 UDP 的服务端/客户端

[本章代码](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch06/README.md)
...
[本章代码](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch07/README.md)
...
- [file_client.c](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch07/file_client.c)
- [file_server.c](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch07/file_server.c)
...
[本章代码](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch08/README.md)
...
[本章代码](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch10/README.md)
...
[本章代码](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch11/README.md)
...
[本章代码](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch12/README.md)
...
[本章代码](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch13/README.md)
...
[本章代码](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch14/README.md)
...
[本章代码](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch15/README.md)
...
[本章代码](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch16/README.md)
...
这个代码可以与 [sep_clnt.c](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch16/sep_clnt.c) 配合起来使用，编译过程和上面一样，运行结果为：

![](https://i.loli.net/2019/01/30/5c513d54a27e0.png)

## 第 17 章 优于 select 的 epoll

[本章代码](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch17/README.md)
...
[本章代码](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch18/README.md)
...
[本章代码](https://github.com/Corner430/TCP-IP-NetworkNote/blob/master/ch24/README.md)
